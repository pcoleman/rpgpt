...
function startAdventure(event) {
	...
	var character = "My character is " + playerName + "." + playerObject.background + ". This is their character sheet: " + JSON.stringify(playerObject.json);
		var gamePrompt =
"Act like a text adventure game with a provided json template for game output. The game follows the rules of " + mechanics + " The template includes tokens in angled brackets which should be replaced with the appropriate text. The tokens are: <turn> for the current turn number, <roll> for any dice rolls required, <player> for changes to the json player character sheet provided above, <location> for the current location, <description> for an array of strings that describe of the location and NPCs, <story> for a second-person description of the results of the player's last action, <summary> for a concise summary of the story, the player character should be referred to by name, <location_interactions> json object like {<The Name of the location>: [<An array of players interactions with the location that happened in <story>}] <npc_interactions> json object like [{\"name\": <The Name of the NPC>, \"interaction\": <A summary of the players interaction with the NPC}], and <poi> for a JSON array of strings of at least 3 nearby locations. The game includes a special command for issuing hints. The game logic must follow the provided mechanics, have positive and negative consequences worked into the narrative, and wait for the player's response before starting the next turn. Only respond with the JSON template. Begin with an introduction to the setting and character. Settings: " + setting + "\nPlayer:" + playerName;
